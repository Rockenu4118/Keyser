...
crypto::asym::ECKeyPair::ECKeyPair()
{
    genKeyPairObj();
    extractKeys();
}

crypto::asym::ECKeyPair::ECKeyPair(nlohmann::json json)
{
    insertPrivateKey(json["privateKey"]);
    extractKeys();
}

crypto::asym::ECKeyPair::ECKeyPair(KeyType type, std::string key)
{
    switch (type)
    {
...
// Modifiers
bool crypto::asym::ECKeyPair::genKeyPairObj()
{
    ...
    // EVP_PKEY_CTX* my_ctx = EVP_PKEY_CTX_new_id(NID_secp256k1, nullptr);

    // if (!EVP_PKEY_keygen_init(my_ctx))
    //     return false;

    // if (!EVP_PKEY_keygen(my_ctx, &keyPair))
    //     return false;
...
bool crypto::asym::ECKeyPair::insertPrivateKey(std::string privateKey)
{
...
bool crypto::asym::ECKeyPair::insertPublicKey(std::string publicKey)
{
...
bool crypto::asym::ECKeyPair::extractKeys()
{
...
nlohmann::json crypto::asym::ECKeyPair::json() const
{
    nlohmann::json json;
...
// Other
void crypto::asym::ECKeyPair::sign(std::string hash, std::string& rSigVal, std::string& sSigVal)
{
...
bool crypto::asym::ECKeyPair::verify(std::string hash, std::string rSigVal, std::string sSigVal)
{
