...
keyser::Transaction newTransaction(unsigned char recipient[], unsigned char sender[], const uint64_t amount)
{
    keyser::Transaction tx;
    // memcpy(mRecipient, recipient, sizeof(mRecipient));
    // memcpy(mSender, sender, sizeof(mSender));
    // mAmount = amount;
    return tx;
}
...
void keyser::hashTransaction(Transaction& tx, unsigned char hash[])
{
    ...
    memcpy(&buf[offset], &tx.mVersion, sizeof(tx.mVersion));
    offset += sizeof(tx.mVersion);
    memcpy(&buf[offset], &tx.mTimestamp, sizeof(tx.mTimestamp));
    offset += sizeof(tx.mTimestamp);
    memcpy(&buf[offset], &tx.mLocktime, sizeof(tx.mLocktime));
    offset += sizeof(tx.mLocktime);
    memcpy(&buf[offset], &tx.mGas, sizeof(tx.mGas));
    offset += sizeof(tx.mGas);
    memcpy(&buf[offset], &tx.mRecipient, sizeof(tx.mRecipient));
    offset += sizeof(tx.mRecipient);
    memcpy(&buf[offset], &tx.mSender, sizeof(tx.mSender));
    offset += sizeof(tx.mSender);
    memcpy(&buf[offset], &tx.mAmount, sizeof(tx.mAmount));

    if (!SHA256(buf, bufsize, hash))
    {
        throw std::runtime_error("Failed to hash transaction");
    }
}
...
namespace keyser
{
    ...
    // {
    //     out << "Time: "      << utils::localTime(data.mTimestamp)   << std::endl;
